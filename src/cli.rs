...
#[command(long_about = "Terminal Jarvis provides a unified interface for managing multiple AI coding tools like claude-code, gemini-cli, qwen-code, and opencode.")]
pub struct Cli {
    #[command(subcommand)]
    ...
    pub async fn run(self) -> anyhow::Result<()> {
        match self.command {
            Commands::Run { tool, args } => {
                cli_logic::handle_run_tool(&tool, &args).await
            }
            Commands::Update { package } => {
                cli_logic::handle_update_packages(package.as_deref()).await
            }
            Commands::List => {
                cli_logic::handle_list_tools().await
            }
            Commands::Info { tool } => {
                cli_logic::handle_tool_info(&tool).await
            }
            Commands::Templates { action } => {
                match action {
                    TemplateCommands::Init => {
                        cli_logic::handle_templates_init().await
                    }
                    TemplateCommands::Create { name } => {
                        cli_logic::handle_templates_create(&name).await
                    }
                    TemplateCommands::List => {
                        cli_logic::handle_templates_list().await
                    }
                    TemplateCommands::Apply { name } => {
                        cli_logic::handle_templates_apply(&name).await
                    }
                }
            }
        }
