...
pub async fn handle_list_tools() -> Result<()> {
  ...
  if !InstallationManager::check_npm_available() {
    println!("Warning: NPM is not available. Most tools require NPM for installation.");
...
pub async fn handle_interactive_mode() -> Result<()> {
  ...
  loop {
    ...
    if !npm_available {
      println!("{neon_blue}Node.js required → https://nodejs.org/{reset}");
      println!();
    }
    ...
    match selection.as_str() {
      s if s.starts_with("AI CLI Tools") => {
        handle_ai_tools_menu().await?;
      }
      s if s.starts_with("Important Links") => {
...
async fn handle_important_links() -> Result<()> {
  ...
  println!("{neon_cyan}Important Links Important Links & Resources Resources{reset}\n");
...
async fn handle_manage_tools_menu() -> Result<()> {
  ...
  println!("{neon_cyan}Settings  Settings & Tools Tools{reset}\n");

  let options = vec![
    "Install Tools".to_string(),
    "Update Tools".to_string(),
    "List All Tools".to_string(),
    "ℹ Tool Information".to_string(),
    "Back to Main Menu".to_string(),
  ];
  ...
  match selection.as_str() {
    s if s.starts_with("Install Tools") => handle_install_tools_menu().await,
    s if s.starts_with("Update Tools") => handle_update_tools_menu().await,
    s if s.starts_with("List All Tools") => {
      handle_list_tools().await?;
      println!("\n{neon_cyan}Press Enter to continue...{reset}");
      let _ = std::io::stdin().read_line(&mut String::new());
      Ok(())
    }
    s if s.starts_with("ℹ") => handle_tool_info_menu().await,
...
pub async fn handle_cache_refresh(ttl: u64) -> Result<()> {
  ...
  match latest_version_info {
    ...
    None => {
      ProgressUtils::warning_message(" No version information available to cache");
    }
  }
