...
use crate::services::{PackageService, GitHubService};
...
pub async fn handle_run_tool(tool: &str, args: &[String]) -> Result<()> {
    if !SUPPORTED_TOOLS.contains(&tool) {
        return Err(anyhow!("Unsupported tool: {}. Supported tools: {:?}", tool, SUPPORTED_TOOLS));
    }
    ...
    match package {
        Some(pkg) => {
            if !SUPPORTED_TOOLS.contains(&pkg) {
                return Err(anyhow!("Unsupported package: {}. Supported packages: {:?}", pkg, SUPPORTED_TOOLS));
            }
...
pub async fn handle_list_tools() -> Result<()> {
    ...
    for tool in SUPPORTED_TOOLS {
        let is_installed = package_service.is_tool_installed(tool).await.unwrap_or(false);
        let status = if is_installed { "✅ Installed" } else { "❌ Not installed" };
        let description = tool_api.get_tool_description(tool).await.unwrap_or_else(|_| "No description available".to_string());
...
pub async fn handle_tool_info(tool: &str) -> Result<()> {
    ...
    let version = if is_installed {
        package_service.get_tool_version(tool).await.unwrap_or_else(|_| "Unknown".to_string())
    } else {
    ...
    println!("Status: {}", if is_installed { "Installed" } else { "Not installed" });
