...
- [ ] If required, run a bounded fallback POC using identical measurements.
...
accounted for. Page 13 follows the bound mode. For `zero-host`, it proves that
page 09 is the only delivery route and that no broker, provider credential,
sandbox endpoint, or billable hosted resource exists.
